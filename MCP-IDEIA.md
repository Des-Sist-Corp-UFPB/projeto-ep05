# Ideia de Servidor MCP — EQ05

**Domínio:** Plataforma de confeitaria (Sweet Delights, Mercado Pago)  
**Data:** 2026-07-01
...
## Servidor proposto: `pedidos-mcp`

### Tools sugeridas

- `catalogo()` — itens disponíveis
- `montar_pedido(itens)` — cria pedido
- `status_pagamento_pix(pedidoId)` — status do PIX
- `rastrear_pedido(pedidoId)` — situação de entrega

### Resources (somente leitura)

- cardápio; status de pedidos

### Exemplos de uso com um LLM

- "Quero um bolo de chocolate e 6 brigadeiros — monta o pedido e me passa o PIX."
...
public class PedidosTools {
    ...
    public PedidosTools(SeuService seuService) { this.seuService = seuService; }

    @Tool(description = "itens disponíveis")
    public Object catalogo(/* params */) {
